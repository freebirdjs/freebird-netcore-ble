...
module.exports = function (netcore, controller) {
    if (!controller)
        throw new Error('Controller should be given when requiring this module.');

    netDrivers = netDrivers(netcore);
    devDrivers = devDrivers(netcore);
    gadDrivers = gadDrivers(netcore);
    ...
    if (drivers.gad === null) {
        ...
        drivers.gad.getReportCfg = typeof gadDrivers.getReportCfg === 'function' ? gadDrivers.getReportCfg.bind(controller) : undefined;
        drivers.gad.setReportCfg = typeof gadDrivers.setReportCfg === 'function' ? gadDrivers.setReportCfg.bind(controller) : undefined;
    }
