...
function writeReportCfg (permAddr, auxId, attrName, cfg, callback) {
    ...
    attrVal = charVal ? charVal[attrName] : null;
    rptCfgInfo = charVal ? _.get(reportCfgTable, [permAddr, auxId, attrName]) : _.get(reportCfgTable, [permAddr, auxId]);
    ...
    attrVal ? _.set(reportCfgTable, [permAddr, auxId, attrName], rptCfgInfo) : _.set(reportCfgTable, [permAddr, auxId], rptCfgInfo);
...
function operateGadAttr (central, type, permAddr, auxId, attrName, val, callback) {
    ...
    if (dev.status === 'offline') 
        return callback(new Error('Device is not online.'));
