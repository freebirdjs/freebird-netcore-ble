...
module.exports = function (netcore, dev, notifData) {
	var char = dev.findChar(data.servUuid, charId),
    ...
    if (data.servUuid === '0x180a') {
        ...
        if (charId === '0x2a29') {
            netcore.commitDevReporting(dev.addr, {manufacturer: charVal});
        } else if (charId === '0x2a24') {
            netcore.commitDevReporting(dev.addr, {model: charVal});
        } else if (charId === '0x2a25') {
            netcore.commitDevReporting(dev.addr, {serial: charVal});
        ...
        if (_.includes(helper.getUuids('public'), charId)) {
            reportFunc(dev.addr, data.servUuid + '.' + charId, charVal);
